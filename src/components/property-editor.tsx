...
export const CopyrightPositionRadioGroup = (
  props: CopyrightPositionRadioGroupProps
) => {
  return (
    <RadioGroup
      label="コピーライトの位置"
      onChange={props.onCopyrightConfigChange}
      selectedValue={props.value}
    >
...
export const PropertyEditor = (props: Props) => {
  ...
  const onFontSizeChange = useCallback<(fontSize: number) => void>(
    (fontSize) => {
  ...
  return (
    <section>
      <H2>編集項目</H2>
      <CopyrightPositionRadioGroup
        onCopyrightConfigChange={onCopyrightPositionChange}
